...
O objeto `translator` é uma [configuração](../../reference/configuration/) que é validada em tempo de execução. Ele é removido do frontmatter antes do início da tradução, para que não afete o resultado final.
